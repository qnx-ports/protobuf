...
TEST(CppGeneratedCode, SetExtension) {
  ...
  void* prior_message;
  {
    ...
    prior_message = hpb::interop::upb::GetInternalMsg(&extension1);
    EXPECT_EQ(false, ::hpb::HasExtension(&model, theme));
    EXPECT_EQ(true,
  ...
  EXPECT_TRUE(ext.ok());
  EXPECT_EQ(hpb::interop::upb::GetInternalMsg(*ext), prior_message);
}
...
TEST(CppGeneratedCode, SetExtensionWithPtr) {
  ...
  auto ext = hpb::GetExtension(model, theme);
  EXPECT_TRUE(ext.ok());
  EXPECT_NE(hpb::interop::upb::GetInternalMsg(*ext), prior_message);
}
